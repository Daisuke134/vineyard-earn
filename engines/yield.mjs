...
// Returns `o` (not just side-effecting) so `run()`'s `return out({...})` branches resolve to the
// actual result object for callers (core/loop.mjs, engines/yield.test.mjs) — not just print it.
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  // NOTE: every branch inside run() already calls out() itself (side-effecting print + return value)
  // — so this entrypoint only needs .catch() for an uncaught rejection; a `.then(out)` here would
  // print the same JSON line a second time.
